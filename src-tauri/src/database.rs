    ...
    connection
        .execute(
            "DELETE FROM activity_events
             WHERE kind = 'blocked' AND detail = 'DNS domain sinkholed'",
            [],
        )
        .map_err(|error| format!("Unable to remove legacy synthetic activity: {error}"))?;
    Ok((connection, path))
}
...
pub fn ensure_blocklist(connection: &Connection) -> Result<Vec<String>, String> {
    ...
    if count == 0 {
        connection
            .execute_batch(SEED_BLOCKLIST_SQL)
